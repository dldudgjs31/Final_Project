...
function check() {
    ...
    if(mode=="created"||mode=="update" && f.upload.value!="") {
		if(! /(\.gif|\.jpg|\.png|\.jpeg)$/i.test(f.upload.value)) {
			alert('5개의 이미지 파일을 선택해주세요.');
			f.upload.focus();
			return;
		}
	}
    
	f.action="${pageContext.request.contextPath}/admin/ncha_banner/${mode}";
	f.submit();
};
    //업로드한 이미지 미리보기
...
function deleteFile(fileNum) {
		var url="${pageContext.request.contextPath}/admin/ncha_banner/delete";
		...
		<div>
			<form name="bannerForm" method="post" onsubmit="return submitContents(this);" enctype="multipart/form-data">
			  <table class="table text-center">
				  ...
				  <c:forEach var="dto" items="${list}">
				  	<tr id="file${dto.fileNum}">
				  		...
				  		<div id="updateImage" style="width: 100px; height: 100px; background-image: url('${pageContext.request.contextPath}/uploads/ncha_banner/${dto.serverFilename}');"></div>
			 ...
			 <table class="table text-center">
			     <tr> 
			      <td>
			        <button type="submit" class="btn btn-primary" style="font-family: 'Jua', sans-serif;" onclick="javascript:location.href='${pageContext.request.contextPath}/ncha_bbs/main';">${mode=='update'?'수정완료':'등록하기'}</button>
			        <button type="reset" class="btn btn-primary" style="font-family: 'Jua', sans-serif;">다시입력</button>
			        <button type="button" class="btn btn-danger" style="font-family: 'Jua', sans-serif;" onclick="javascript:location.href='${pageContext.request.contextPath}/ncha_bbs/main';">${mode=='update'?'수정취소':'등록취소'}</button>
